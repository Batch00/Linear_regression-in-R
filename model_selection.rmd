...
---
title: "STAT340 HW11: Model selction"
...
```{r}
lin = sum(linear_model$residuals^2)
quad = sum(quad_model$residuals^2)
cube = sum(cube_model$residuals^2)
...
predicting stopping distance as the square of speed is the best because it has the lowest RSS of the three models
...
```{r}
qc = lm(dist ~ 1 + speed + I(speed^2), data=cars)
```
...
```{r}
lq_r = sum(qc$residuals^2)
c(lin, quad, lq_r)
```

***

yes the model that predicts stopping distance from squared speed and linear speed is better than the two by themselves. This is not very surprising because I have we are still only using two variables and it does not cause the model to be overfitting. 
...
for ( i in 1:nrow(cars) ) {
  ...
  errors[idx,]$Error <- (m1.pred - leftout$dist)^2 ; 
  
  # Fit the quadratic model, then evaluate.
  m2 <- lm(dist ~ 1 + I(speed^2), train_data );
  m2.pred <- predict( m2, leftout );
  idx <- (errors$Row==i & errors$Order==2)
  errors[idx,]$Error <- (m2.pred - leftout$dist)^2; 

  # Fit the combined model, then evaluate.
  m3 <- lm(dist ~ 1 + speed + I(speed^2), train_data );
  m3.pred <- predict( m3, leftout );
  idx <- (errors$Row==i & errors$Order==3)
  errors[idx,]$Error <- (m3.pred - leftout$dist)^2
}
...
According to LOOCV, the best model is the quadratic model. This is not suprising because I have learned that you have to be very careful and it can be easy to overfit your model so it does not surprise me that this is the result.
...
generate_X <- function( n, p ) {
  mat = matrix(data = rnorm(n), nrow=n, ncol=p, byrow=TRUE)
  data.frame(mat)
}
...
generate_coefs <- function( p, k ) {
  ...
  for (i in 1:p) {
    if (k >= i & i >= 1) {
      beta[i] = sample(c(-1,1), 1, replace=TRUE)
    }
    if (p >= i & i > k) {
      beta[i] = 0
    }
...
df = generate_X(200, 10)
df = generate_responses(df, generate_coefs(10,3))
...
for( i in 1:10 ) {
  ...
  # TODO: write code to fit a model to this "subsetted" data frame. compute the fitted model's RSS; store it in the i-th entry of model_rss,
...
```{r}
ggplot(NULL, aes(x = 1:length(model_rss), y=model_rss)) +
  geom_line( size=1)
```

***

This makes sense because we made the data frame so that The first three predictor columns should be "predictive" and the graph seems to level off right at 3. So that means that the model begins to be overfitted after the third predictor.
...
lasso_lambda0 <- glmnet(x_df, y_col, alpha = 1, lambda=0)
lasso_lambda1 <- glmnet(x_df, y_col, alpha = 1, lambda=.1)
lasso_lambda2 <- glmnet(x_df, y_col, alpha = 1, lambda=.2)
lasso_lambda3 <- glmnet(x_df, y_col, alpha = 1, lambda=.45)
...
the LASSO select *only* the three "predictive" coefficients when lambda is about between .2 and .45
