...
Yes I do see a linear trend and believe the slope to be about 1.5
...
catamaran_lm <- lm(spending ~ 1 + litter,
                      data=catamaran)
...
An increase of one additional pound of litter per year is associated with about 1.6 more dollars spent per year on Catamaran products
...
An increase of one additional three-pound unit of litter is associated with about 4.8 more dollars spent per year on Catamaran products
...
An increase of one additional twenty pound bag of litter is associated with about 32 more dollars spent per year on Catamaran products
...
No the QQplot shows that the residuals of the model are approximately normal
...
qqnorm(catamaran_lm$residuals)
qqline(catamaran_lm$residuals, col='red')
...
confint(catamaran_lm, c('litter'),level=0.95)
...
***
Since the 95% confidence interval is not crossing zero that means there is a small chance that Beta_1 woukd = 0 so we reject the null 
 the p-value associated to our estimate of the coefficient of lead levels is quite small therefore We reject the null hypothesis
...
summary(catamaran_lm)$coefficients
...
***
the length of vector y will be n. beta_0 is eps and beta_1 is -1
***
...
```{r}
df = generate_linear_data(100,.25)

ggplot(df, aes(x=predictors, y=responses))+
  geom_point()
```

***

Yes the data looks approximentely linear, The slope is about .5 and intercept about -1
...
This graph is a lot more spread out and seems more random and is hard to determine whether or not it is linear unlike the previous one
...
This plot has points that are a lot closer together and it very much appears to be linear
...
generate_and_estimate <- function( n, sigma2 ) {
  ...
  data_lm = lm(responses ~ 1 + predictors, data=data)
  beta1hat = unname(data_lm$coefficients[2])
  return (beta1hat)
}
```
...
```{r}
sigma_range = seq(.25,4,.25)
vars = rep(NA, length(sigma_range))
for (i in 1:length(sigma_range)) {
  var = estimate_beta1hat_variance(100, sigma_range[i], 1000)
  vars[i] = var
}

ggplot(NULL, aes(x=sigma_range, y=vars)) +
  geom_point()
```
...
Yes this does agree with my plot because my plot makes it look like the variance of our estimator $\hat{\beta}_1$ as a function of the observation error variance $\sigma^2$ behaves in an increasing manner and it increasing exponentially Just like mathmatical interpretation shows.
